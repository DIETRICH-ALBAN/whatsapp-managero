                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {testimonials.map((t, i) => (
                        <motion.div
                            ...
                            whileHover={{ y: -10 }}
                        >
                            <Card className="h-full bg-card border-border backdrop-blur-sm p-8 hover:bg-card/80 transition-colors">
                                <div className="flex gap-1 mb-4">
                                    {[...Array(5)].map((_, starI) => (
                                        <Star
                                            key={starI}
                                            className={`w-5 h-5 ${starI < t.rating ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground/30'}`}
                                        />
                                    ))}
                                </div>
